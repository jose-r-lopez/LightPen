...
public class plg_PortScan implements Plugin {
    ...
    public ArrayList<Error> run(URL dir) {
        ...
        try {
            Class<?> cl = Class.forName(pluginPath + ".impl.PortScanImpl");
            implementation = (IPluginImplementation) cl.newInstance();
        } catch (Exception e) {
            throw new RuntimeException("Unexpected failure on PortScan plugin:" + e.toString());
        }
        //     }
    ...
    public String printErrors(ArrayList<Error> errores) {
        ...
        if (!errores.isEmpty()) {
            ...
            ans += "<th>" + ctxD.getContext()
                    .getString(R.string.PortScan_protocol) + "</th>\n";
            ans += "<th>" + ctxD.getContext()
                    .getString(R.string.PortScan_servicename) + "</th>\n";
            ans += "<th>" + ctxD.getContext()
                    .getString(R.string.PortScan_servicedescription) + "</th>\n";
            ans += "</tr>\n";
            for (Error e : errores) {
                ans += "<tr>\n";
                ans += e.printError();
