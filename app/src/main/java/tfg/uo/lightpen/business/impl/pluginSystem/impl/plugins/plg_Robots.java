 ...
 */
public class plg_Robots implements Plugin{

    //private static final String TAG = "ROBOTS";
   ...
   //     if (implementation == null) {
            try {
                Class<?> cl = Class.forName(pluginPath + ".impl.RobotsImpl");
                implementation = (IPluginImplementation) cl.newInstance();
            } catch (Exception e) {
                throw new RuntimeException("Unexpected failure on Robots plugin:" + e.toString());
            }
   //     }
    ...
    public String showDescription() {
        ...
        return ctxD.getContext().getString(R.string.rs_description);
    }

    @Override
    public String showName() {
        return ctxD.getContext().getString(R.string.rs_name);
    }

    @Override
    public Double showVersion() {
        return Double.parseDouble(ctxD.getContext().getString(R.string.rs_version));
    }
